...
const selectSession = (sessionId) => {
    ...
    const sessionRef = dbRef(db, `trainerInfo/emilie/sessions/${sessionId}`);
    ...
    if (selectedSession) {
        ...
        const sessionBookedRef = dbRef(db, `trainerInfo/emilie/booked-sessions/${sessionId}`)
...
<template>
    <h1 v-if="!showTrainerView">Emilies Kalender</h1>
