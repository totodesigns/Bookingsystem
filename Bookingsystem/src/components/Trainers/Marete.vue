...
const sessionsRef = dbRef(db, `trainerInfo/marete/sessions`)
...
const selectSession = (sessionId) => {
    ...
    const sessionRef = dbRef(db, `trainerInfo/marete/sessions/${sessionId}`);
    ...
    if (selectedSession) {
        ...
        const sessionBookedRef = dbRef(db, `trainerInfo/marete/booked-sessions/${sessionId}`)
...
<template>
    <h1 v-if="!showTrainerView">Maretes Kalender</h1>
